...
- **Never write engine output to the cache tuned for one occurrence kind.** A cache record is `{h, src, en, engine}` with no kind, so one value serves every occurrence of that hash — comment, string literal, raw string. Anything depending on `kind` or on surrounding bytes belongs at splice time. This is why the plan's Task 14 bulk LLM pass is closed rather than pending. ([ADR-0006](docs/decisions/0006-close-task-14-bulk-llm-pass.md))
- **Masking a region means it won't be translated, not that it's independent of what is.** A heading anchor derives from heading text; masking the anchor while translating the heading silently breaks the link. Any masked-but-derived target needs a post-splice repair and a gate check. ([ADR-0007](docs/decisions/0007-markdown-masking-policy-and-derived-target-repair.md))
