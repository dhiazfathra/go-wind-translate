...
**Tasks 1-13 of the [implementation plan](docs/superpowers/plans/2026-08-14-go-wind-zh-en-translation.md) are done, and Task 14 is closed as descoped ([ADR-0006](docs/decisions/0006-close-task-14-bulk-llm-pass.md)) — 162/162 tests passing.** All 11 `go-wind*` repos have been translated, reviewed, and squash-merged (10 target repos on `chore/i18n-en-default`, this tool repo on `master`). The fan-out and review passes surfaced and fixed 19 real bugs in `gwt` itself — see [HANDOFF.md](HANDOFF.md) for the full list, including two recurring runtime-i18n corruption patterns (self-referential language labels, locale-keyed message catalogs) guarded against via `dictionary.tsv` identity pins and a `classify.py` glob, a comment-splicing gap (`gwt/quality.py`) that glued acronyms and full-width punctuation directly onto translated words wherever Chinese needed no space but English does, an MT-quality miss (`180天` → `180Tian`) fixed with a dictionary pin, per-language string-literal escaping so a `.sql` literal is no longer escaped by Go/TypeScript rules, and translated headings orphaning their own in-page anchors (32 dead links across two repos, invisible to the gate until it gained a `broken_anchors` check).

**One thing is still open**, tracked in [HANDOFF.md](HANDOFF.md)'s "What's left": every tool-level fix above landed *after* the target repos' PRs merged, so their translated output still carries the defects the tool now prevents. Propagating the fixes is a pipeline re-run over those repos — free and offline, since the cache is warm. Everything that re-run was blocked on is now resolved.
